# -*- coding: utf-8 -*-
"""边界感知层 v1（古典 CV；HSV 口维持旧栈值，放宽的前置条件与两次试错见 _HSV_LOW 注）：驾驶帧 → BoundarySummary 摘要。

**分层归属**（control-route §一「边界感知」行；契约消费方是校验层与横向位移代价，
**路缘不进目标候选集、不参与评分**——设计稿 v2 §三对审查 §一.2 的裁定）。

**与旧栈的差异是升级不是重写**（HSV 口、形态学去噪、左右分类三件沿用旧栈值；
HSV 放宽的实测依据与回退理由见 _HSV_LOW 注）：旧栈没有标定，只能靠
...
# 黄色范围：旧栈 [20,80,80]-[30,255,255]。**2026-09-22 两次试错记录**：
# ①坏帧验尸定罪本作路缘实测 H=9~17/S=47~74，旧口在暗色场景整段漏检（双侧率 0%）；
# ②当晚放宽到 [8,45,80]-[32,255,255] 后覆盖率 97-99%，但"最左/最右段"选择器无差别
#   捞进同色系建筑/护栏（road_offset 极值 ±7 车道=假路中心，实机表现为醉驾摆动）——
#   颜色分不开线 vs 建筑，几何选择器（x_lane 聚类+路宽配对）是前提，而聚类在偏航下
#   会切碎扫描线、配对窗会硬编码路宽——属边界层 v2 设计题，未做前宽口不得上岗。
# 暂回旧口：好场景可用、坏场景如实降级（sides==0→闭环退开环），不喂毒。
