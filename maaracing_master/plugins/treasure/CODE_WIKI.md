...
- **链内进度写 trace**（`_egg_chain_trace`，事件名 `egg_chain`）：链阻塞在决策段内，`_tick_once` 的帧 trace 在链期间不再产生——不补记则整段领取过程在 trace 里是空白，事后无法回溯「走到哪一步、哪一步失败」。记 `start`（含 `click_mode`/`intent_mode`，可直接回答「为什么没领到」）/ `click` / `click_skipped` / `click_timeout` / `panel_opened` / `claim_done` / `give_up` / `error`。链内失败路径同时收口到统一出口 `_give_up`（日志 + trace + 离场），不再各写各的。
