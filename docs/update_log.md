...
### 暂存（未发布 · 待并入下一版本）日志通道分离：环形缓冲 + 分档落盘 + 保留策略 🔧

- **性质：** 未发版变更（`core/logger.py`、`core/sidecar.py`、`apps/.../frontend/app.js`、`tests/test_logger_channels.py`（新）、`docs/CODE_WIKI.md`；master 直接提交、不 tag）
...
- **落盘分档 + 保留：** 一次开启生成一个会话组、两个文件——`MaaRM_<ts>.log`（INFO/WARNING/ERROR，可安全上报、不含本机绝对路径）与 `MaaRM_<ts>.debug.log`（DEBUG/TRACE，排查用）。两档各自独立句柄 + 大小轮转（16MB×4 份上限）+ 启动时按会话组保留清理（默认保留最近 20 组，只清 `MaaRM_<ts>` 形态文件、不碰 sidecar_stderr.log、不删活动会话）。

- **通道调整：** `log()` 新增 `channel` 参数 + `set_channel_level`/`clear_channel_level`，支持按子系统独立调级（通道管「要不要打」，级别管「打到哪个文件」，两者正交）；旧调用零改动。

- **写盘正确性：** 每条日志独立 open/close 改为持单句柄 + `Lock` + 行缓冲，省掉每行两次系统调用且跨线程写行完整；退出路径 `close()` 关句柄 flush 尾部。

- **新增单测：** `tests/test_logger_channels.py` 17 条（环形回绕不重不漏 / 截断标志 / 多线程并发写行完整 / 分档落盘 / 轮转与备份计数 / 会话保留只清旧组 / 通道级别覆盖与回落）。

- **真机端到端（GUI 区块卡片渲染、长跑 RSS、开两局分档保留验证）** 依赖游戏 + ViGEmBus 驱动，待用户真机复验；单元/契约层面已全绿。
