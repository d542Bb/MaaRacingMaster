...
- **实测结论：** ① **一场之内就会换位**——`20260917_220706_p1` 一场换两次；我方「得分速度」的读数随本机面板从上档移到下档（`rate_a` 24 行 / `rate_b` 39 行，与侧别段一一对应），另一场 `20260917_220453_p1` 全程在下档（61 行）。故**固定槽位框 + 块色成对判**两态都覆盖，**不需要随敌我平移、也不需要两套状态框**。② 换位**是对滑不是瞬切**：过渡约 10 帧里两块面板互相错开、同时在场，**过渡期任何固定位置上的读数都可能是过路内容** → 须整拍弃权。
