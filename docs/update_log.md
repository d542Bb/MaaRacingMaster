...
- **问题：** 出价回合里机器人会反复退回报价（`判定假下降沿（未提交成功）→ phase→wait_first`），表现为「点了出价但游戏像没反应」——实际游戏已接受提交（玩家面板与按钮都显示已出价），是机器人**读不出提交成功的铁证**。根因是 `bid_main_btn_label` 的 ROI 过窄（`x=[0.4314, 0.5240]`，而按钮粉底实测横跨 `x≈0.3883–0.5664`），只盖住按钮横向中段：文案短（`出价` / `等待出价` / `已出价：0`）时恰好框得下，文案随金额变长后（`已出价:353,200`）**左右两头被切**，OCR 只读到 `价：353,2`。raw 帧实证（会话 `20260915_150432` 帧 0509/0542/0587）。

- **为什么后果是「不响应出价」：** `module.py` 有一条自愈——`wait_first` 中若按钮 OCR 读到「已出价」，判定我方实际已提交、回 `wait_result` 继续读公开报价（注释写明「没有这一步…会错过整个公开报价窗口 → 快照缺失 → 后续回合策略退化为 observe」）。该判据完全依赖 label 里出现「已出价」三个字，被切字后永不成立，自愈形同虚设。

- **修法：** ROI 横向加宽到按钮本体 `x=[0.3947, 0.5609]`（y 微调）。扫描实测：左右各扩 0.030–0.060 均可读全，到 0.080 会把左侧倒计时数字吃进来（读成 `2已出价:353,200`）。跨三个会话抽 109 帧比对：新增读数只有两类——正是想要的 `已出价:274,700` / `已出价：256,000`，以及非出价页的邻域文本（`数码潮突藏品` 等，三个消费点都不匹配，落回「保守等待」，与改前同等）。三帧复测：`已出价:353,200` / `等待出价` / `出价` 全部读对。

- **与页面门控无关：** `bid_main_btn_label` 的 `stages_for` 为 `None`（无页面约束），且走同步 OCR 直读，不经过 OCR worker 与闸②。

- **测试期望同步：** `test_navkit_runtime_golden.py` 的 `GOLDEN_OCR` 跟着更新为新 rect；`test_navkit_studio_save_flow.py` 的「改一位小数只动一行」断言原本写死 `0.4324`（旧 rect 推导值），改为由本次改动推导——否则 ROI 一调就因无关原因变红。

- **遗留：** 本次顺带查明「假下降沿」与「按钮自愈」两条判定会对同一事实（我方是否提交成功）下相反结论并周期振荡（实测 2 秒一轮，周期即 `SUBMIT_ANIMATION_BUFFER_MS`），代价是公开报价窗口空转——判据分析与实测证据见域 `CODE_WIKI.md` §9「假下降沿与按钮自愈的判据互斥」。本 ROI 修好会让自愈稳定生效，该振荡更易被点着，需另行处置。
...
### 暂存（未发布 · 待并入下一版本）行为守则新增「排查与修复纪律」📄

- **性质：** 未发版变更（纯文档：`AGENTS.md`、`docs/update_log.md`；master 直接提交、不 tag）

- **动因：** 既有守则约束了「去哪查」与「问题怎么分级」（C 类去写最小实验），但没有一条约束**修复循环本身如何收敛**——模块在个别阶段出错时，改动容易持续加码在该模块内部（兜底 / 重试 / 上限 / 特例 / 旋钮），不先确认该模块在多数场景本就正常，于是补丁叠在错误的层上、形态越修越畸形。最近的结算 OCR 门控即为实例：真正的病灶是「阶段判定滞后」，而加固消费侧门控无效——守的是过期的门。

- **新增四则（`AGENTS.md` §排查与修复纪律）：** ①**反常即差分**——先立工作基线（正常 N / 失败 M），把「失败场景与正常场景的差异」写成假设先验，「这段逻辑本身写错了」降为最后才验的假设；②**修在判据，不修在症状**——兜底 / 重试 / 上限 / 特例 / 旋钮属待解释动作，答不出「通用路径为何覆盖不到这一例」即停止加码，通用路径只在个别场景失效时改边界的输入 / 状态来源（判据是否成立、门控是否守过期的门、真源是否滞后）；③**同一方向连改三次未收敛即换轴**——换假设的种类，或停手报告已排除范围与最大疑点，由用户决定是否扩大改动面；④**修复条目自证未打穿工作基线**——`docs/update_log.md` 修复条目写明基线数字与差异假设，回归锁含原本正常场景的用例并以全量 `pytest` 绿收尾。

- **归宿依据：** `docs/adr/README.md` 准入表把「纯行为守则 / 工作方式」路由到 `AGENTS.md`（ADR 不收）。规则只留通则，案例留在 `docs/update_log.md` 与对应域 CODE_WIKI §9，不复制。
...
- **判定锚点从 `active` 派生，不新增 policy 字段：** 取「标志锚点」——`active_for(stage)` 中归属阶段就是本阶段的模板锚点。`active` 里混着邻页锚点（「领取分红」的 active 含 `daily_high_banner`，其归属阶段是「结算弹窗」），只有归属本阶段的才是该页的现场证据；回合族阶段的标志锚点归属 `__round_phase__` 哨兵（`smart_bid_btn` / `round_big_banner`），由 `detector.is_round_stage` 认领——该判据是「哪些阶段属回合族」的唯一定义，门控侧共用，不各自表述。与 `stages_for`/`ocr_for` 是同一份真源的另一面。

- **令牌词汇：按页族，不推导回合号。** 探针命中归属具体阶段的锚点 → 该阶段名（`settle_title`→「领取分红」、`result_banner`→「中标结算」）；命中回合族锚点 → 返回 `__round_phase__` 哨兵本身，意为「本帧在出价面板页，具体第几回合不推导」。回合号对本闸是**无关维度**：`stages_for(bid_playerN)` 本就返回 5 个回合名、任一都放行，门控只问页族。而回合号在 detector 里由 `_last_round` 提供，那是**观察线程**按周期维护的跨帧状态——探针（worker 线程）去读它，等于把刚拆掉的跨线程陈旧结论从另一个门请回来；该字段为空时更会直接返回 None，并把自带回合号的横幅一并短路（出价面板锚点 `smart_bid_btn` 优先级 80 高于横幅 70）。门控侧 `_page_token_matches` 收下哨兵，「哪个阶段属回合族」与锚点归属共用 `detector.is_round_stage` 一处判据，不各自表述。
...
- **为什么不做完整阶段判定：** 实测否决。全量扫描（13 锚点 × 13 尺度）p50 140 / max 249 ms，而 OCR worker 单帧约 110 ms、决策帧间隔 p50 110 ms——每帧全量会让 worker 吞吐减半并触发 latest-only 丢帧。标志锚点集命中即短路：结算页 2.0–3.5 ms、出价页 ~2 ms（帧立即命中）；判不出本页的帧要扫满该页族的锚点集，p50 ~20 ms。

- **不碰实例状态：既不写，也不读。** `probe_page` 走 `_scan(record=False)`，不写 `_last_hit_roi_key` / `_last_detect_scores` / `_last_round`——这三个字段的生产者约定是观察线程，而决策线程正在读它们（结算弹窗分类 `_run_ocr`、`popup_kind`、trace）；同样不**读**其中任何一个（回合号那个坑见上条「令牌词汇」）。`_last_raw_stage` 保留给 `_run_appraiser_choice` 的选师准星，只是不再供 OCR 令牌。

- **性能：** 截图次数、帧拷贝次数、ROI 识别次数均不变；每帧新增**一次**标志锚点匹配（两条通道共用同一令牌，不重复跑）。离线实测该匹配（出价族 2 锚点）：命中 p50 **2.1 ms**、未命中 p50 **19.9–20.6 ms**；未命中帧占比等于「`_current_stage` 与画面不一致」的帧占比（转场 + 管线滞后窗口），并非只发生在转场——会话 `20260915_140500` 局内 540 心跳里有 310 帧判不出本页（57%），按帧加权均值约 15–18 ms，对照 OCR 单帧 p50 ~22 ms。**该耗时不计入上报的 `duration_ms` / 「第二段耗时」**（探针在 `t0` 之前调用）。它对读取时效的净影响**实测为零**：逐帧 `时效 = 消费 − 捕获` 分布在三版之间无差（改前 p50 111 / p90 220 ms，旧帧内 115 / 223，本版 110 / 211）——worker 单帧 OCR ~26 ms + 探针 ~16 ms 仍远低于 110 ms 决策节拍，成本被节拍吸收。注意性能汇总行里的 `时效 p95` 跨会话抖动大（113 / 222 / 224 ms），不可用作版本对比依据。门控只丢弃不重试、不触发补帧；错页帧被丢后下游日志与状态写入不再执行。

- **工作基线（自证未打穿）：** 口径 = 探针与真值用**两个独立 detector 实例**（等同生产：worker 侧探针不吃观察线程的任何状态；同一实例「先全量 detect 再探针」会把 `_last_round` 喂新鲜，那是生产不存在的口径，本次首轮验收正是这样漏过的）。三个真机会话抽样：出价帧 **167/167** 判出、结算帧 **34/34** 一致、跨族放行 **0**（`20260915_140500` 出价 16；`20260915_090037` 出价 63 + 结算 8；`20260915_121005` 出价 88 + 结算 26）。脚本 `tools/experiments/frame-page-token/verify_round_baseline.py`（判定正确性与成本另见 `probe.py`）。回归锁 `tests/test_treasure_ocr_page_gate.py` 29 例（真源 2 + 门控行为 10 + 帧内判定 7 + 判据派生与无副作用 7 + 端到端 3）；新增的两条「回合族令牌不读 `_last_round`」已验证在旧行为下会红。全量 `pytest 542 passed`。
...
- **遗留：** ①「中标结算」阶段 `settle_*` 读数取证；②`page` 死字段（建议删除或修正声明）；③`settle_profit` 相对下限 `max(H)/20` 误杀 8/8 个真实微小利润；④`auction_result` 漏读（9/6 21 场，其中 6 场实际中标被记 0 胜）；⑤投递侧「要不要投递」仍基于去抖的 `_current_stage`——转场后可能多投几帧，帧内判定会把它们判为 None 丢弃（只浪费一次识别，不影响读数正确性）；⑥探针在「阶段与画面不一致」的帧上要扫满本页族锚点集（p50 ~20 ms，占比即不一致帧占比）——该耗时不计入上报的 `duration_ms`，实测也不改变读取时效（见性能条）；日后若要压缩，应从判据入手（例如让投递侧少投不一致帧），不加时长旋钮。详见 `docs/plan/archive/bid_audit_20260915/结算OCR页面门控方案.md` §七。
...
- **问题：** 卡第二分支原口径 `lower = 第三名 + 缓冲`、`upper = min(M − u, cap, 余额)`，`lower ≥ upper` 时回退 `competitor + u`，把「对手历史峰值 M」到「买入线」之间整段可盈利区间让掉。2026-09-05 后 61 场落盘审计（`docs/plan/archive/bid_audit_20260915/`）显示：59 场可复算对局中 11 场（18.6%）存在买入线内本可拍中的机会，合计潜在利润 750,277；整链静态仿真现行口径拍中 7 场、利润 417,194。
...
- **验证边界（真机待复验）：** 静态仿真为**上界估计**（对手出价按落盘值回放，不建模「我方出价不同 → 对手下一轮反应不同」）。V4 会让 R3 起我方成为公开盘面高位，需真机验证对手是否跟涨。生产代码回放与仿真一致：拍中 17 场 / 利润 647,362 / 零亏损（`docs/plan/archive/bid_audit_20260915/verify_v4.py`）。
