...
### 暂存（未发布 · 待并入下一版本）出价页页面令牌补第二类同帧证据：与投递时回合号互相印证 🔧

- **性质：** 未发版变更（`plugins/treasure/module.py`、`plugins/treasure/detector.py`、`tests/test_treasure_ocr_page_gate.py`；master 直接提交、不 tag）

- **问题（工作基线 + 差异假设）：** 4 人报价数字**一次都没进过状态**。工作基线：真机 5 个会话（`20260915_181815` / `194459` / `200352` / `201222` / `201920`）里，四个报价槽「消费/输出/命中」的**命中数全场 ≈0**（`201920` 逐回合 `45/45/0`、`48/45/1`、`47/45/0`、`44/44/0`），13 条「快照构建等待」全部未固化 → 对手报价表恒空 → 快照建不起来 → 相位停在 `wait_result` → 策略只能走「智能出价=抄现价」。差异假设（先验，不是「逻辑写错了」）：**同一段代码在面板打开期一直正常**（`201920` 面板开帧令牌正常，出价页锚点得分 1.000/1.000/0.911），失败场景与正常场景的差别只有一个——**出价面板关着**。验证结论：报数字只在「4 人都已出价、面板关闭」之后才逐个显示，而页面令牌的两个标志锚点（`smart_bid_btn` 面板按钮 / `round_big_banner` 回合横幅）此时都已消失 → `probe_page` 恒 `None` → 闸②`fail-closed` 把整段公开报价丢掉。实证：`201920` 本帧页=`None` 817 次，被丢键 `bid_player4` 与 `bid_result_amount_box` 各 482 次、`bid_player1..3`/`player_name1..4`/`round_label_area` 各 350 次；debug 帧复算（仓库自身 OCR）同一帧读出 P1=122,100 / P2=250,000 / P3=163,100，而令牌是 `None`。时间轴不是原因：同窗口 OCR worker 运行次数 1→68 一帧没停，时效 105→220 ms 远低于 `OCR_MAX_AGE_MS`。

- **修复口径：** 令牌仍由 worker 对**同一帧**现场产出，只增加一类证据，消费侧（`_ocr_filter_by_page` / `_page_token_matches`）与闸序完全不动：第一类（模板标志锚点）命中即止；第一类缺席**且本批信号属回合族**时，用同帧回合小字（`round_label_area`）解析出的回合号与**投递时快照的 `_round_no`**（`_ocr_push` 存下的、即阶段标签当时的答案）互相印证，一致才产出 `__round_phase__`（`detector.confirm_round_page`，复用既有 `_detect_round_full` / `_round_no_from_text`，不另立第二套读法；第 6+ 附加回合按 `set_stage` 的 clamp 口径比）。改判据不改症状：未回合族批次不启用该兜底，结算/分红页仍只认标志锚点。

- **为什么是「互相印证」而不是二选一：** 两个来源各有各的前科——阶段标签曾在结算转场慢半拍（画面已是大厅仍报「领取分红」，455 场中 50 场把 300000 落盘）；本帧文字只能证明画面上写着某个回合号，不证明这是哪一页。要求两者同时指向同一回合，任一不一致即回到 `fail-closed`：公开报价窗口（标签=第1回合、小字=第1回合）放行；300000 那类（同帧认不出结算页）仍拦；回合切换瞬间（标签旧、画面新）也拦，顺带覆盖「末尾缺 0 的残缺值（209,500→20,950）」那一类。

- **顺带对齐的文档漂移：** `_run_ocr` 的 docstring 仍写「出价阶段仅在面板已开（S3）时投递」，而代码自 `wait_result` 起就一并投递（该窗口正是对手报价的唯一来源），已按代码更正；`_ocr_filter_by_page` 的「闸①在 round_no 为 None 时放行」改为「两侧回合号都为 None 时放行」——原措辞歧义曾被读成「代码与注释矛盾」，复核代码后确认行为本身无矛盾，只是没说清是哪一侧。

- **工作基线自证（离线复验，调用生产函数本体）：** `tools/experiments/ocr-gate-2026-09-15/probe_fix.py` 以 `201920` 的 9 张实机帧重放——出价页 5 帧（面板开 1 + 公开报价 4）令牌 `None`→`__round_phase__`，其中 3 帧把 P1~P3 读数从「收 0 条」变为「收 1~3 条」（0325 一帧收下 122,100 / 250,000 / 163,100）；**原本正常场景未被打穿**：面板开帧令牌与改前一致（仍走第一类证据，不额外跑回合小字 OCR）；未中标横幅（小字读成「第40」解析失败）、结算页、鉴宝大厅页 3 帧仍为 `None`。回归锁 `tests/test_treasure_ocr_page_gate.py` 35 例（原 29 + 新 6：印证成立出令牌 / 回合号不等仍 `fail-closed` / 非回合族不问印证 / 第一类优先不多跑 OCR / 真 detector 印证口径含 clamp / 印证令牌过闸②进消费）；全量 `pytest 554 passed`。

- **真机复验（2026-09-15 21:10 会话 `MaaRM_20260915_211002`，5 场 / 6.1 MB / 54431 行，trace `debug/treasure/20260915_211008/trace.jsonl`）：** 四个看点全部达标。①新观测行按预期只在这段窗口出现：`页面令牌产地` 共 44 次切换，其中 **`回合小字印证` 12 次**、`模板锚点` 22 次、`无` 10 次；②四个报价槽的**命中数由 0~1 涨到 3~6**（R1 6/5/4/6、R2~R4 各 5~6），**槽固化 44 条**（11 个回合 × 4 槽全部固化，改前为 0）；③**`上一轮快照已构建` 13 条**（改前 0 条，全部停在「快照构建等待…未固化」）且都带对手报价，策略由此拿到真实对手数字——同会话可见 `target_second` 目标价输入（如 `意图: [target_second] 输入 3（已 空 → 目标 338,688）`）与 `意图: 出价按钮已亮（OCR=出价）→ 点出价` 81 帧，不再是「整场反复点智能出价抄现价」；④`已应用` 2184 次（改前 242），跨页丢弃的 `None` 由 817 降到 412，且**「阶段=第N回合出价 期间发生的丢弃 = 0」**（412 次全部落在结算/分红与转场时段：其中 207 次是关键通道的 `bid_result_amount_box`+`bid_player4` 在结算页被正确拒绝、205 次是结算 4 键在认不出页的帧上被拒）。时效 p50 214 / p90 223 / max 444 ms，**超 800 ms 样本 0 条**；超龄丢弃 6、过期丢弃 6；`识别健康 ok（bid_window 0.0%）`。trace 侧：`S4_wait_result` 736 帧 / `S3_*` 1196 帧 / `S4_fake_fallback` **1 帧**（改前一局 19 次），**无任何「同一相位连续 ≥150 帧」的段落**（改前出现过 618 帧纯 `S4_wait_result` 的静默锁死）。

- **同批处置：真机复验带出的跨场槽位残留（随本次一并修复）。** `_reset_round_state`（回到大厅/选场次时调用）清了 `_player_bids` / `_last_round_snapshot` 等，但**没有清 `_bid_slots` / `_bid_slots_round`**；而 `_consume_ocr_result` 的槽重置只在 `_bid_slots_round != r` 时触发——**回合号相等就不重置**。于是「上一场最后一回合恰为第1回合」时，下一场第1回合沿用上一场已固化的四个槽：实测 21:11:21（第 2 场第 1 回合开始）四个槽仍是第 1 场的 `✓150,900 / ✓208,800 / ✓550,000 / ✓178,000`，`OCR 199次` 不再增长（动态 keys 已把固化槽剔除），第 2、3 场的 R1 报价一次不读、R1 快照直接用上一场数字（`epoch#1 R1 H=216,600 我方=150,900 对手=(208800, 550000, 178000)`——H 是本场的，四槽是上一场的）。本会话 5 场中第 2、3 场中招；第 3 场打完 R3 后，第 4、5 场的 R1 因回合号不等而正常重置。**该缺陷在改前不可达**（那时任何槽都不会固化），属本次修复生效后暴露的既有问题。改法：`_reset_round_state` 一并 `_reset_bid_slots()` 并把 `_bid_slots_round` 置 `None`——复用既有重置函数而非清成空 dict，维持「四槽恒在」约定（消费侧 `_bid_slots[pid]` 按下标直取）。回归锁 `tests/test_treasure_bid_phase_recovery.py::test_reset_round_state_clears_locked_bid_slots`，已按仓内口径验证在旧行为下会红（`assert 1 is None`）。全量 `pytest 555 passed`。
