...
- **撤回两条结论（本轮最重要的修正）：** ① ~~「里程是累计里程、同段内单调递增」~~ ——逐帧看左侧面板，里程在 338 → **259** 处掉头（291/338/259/278/283），**该计数器会重置**，重置条件未知；合计随之一同重置（合计 = 里程 + 30×超车 每次读数都成立，两者同源），而**右上双方分数才是单调递增的累计值**（6 → 3693、零违例），两个"分"不是同一个量。② ~~「里程 + 超车 只占总分约 12%」~~ ——该比例用"会重置的滚动计数 ÷ 累计总分"得出，分子分母不同期，比例无意义，**撤回**；方向性结论（累计总分绝大部分不来自左侧两项）仍与 §4.2「主要加分项＝金币」一致，但具体占比给不出。教训记为第 5 条复核判据：**"读数对"不等于"语义对"**——三格每格都读对了，错的是把它解释成累计值。
