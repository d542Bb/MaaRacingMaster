...
### 暂存（未发布 · 待并入下一版本）极限动作触发反查：**超了必得分，升级与否只分离在横向贴近**（逐帧事件时钟）🔬
...
- **结论②（唯一显著分离 = 横向贴近）：** 极限组"最近接近横向距离"中位 **80 px** vs 普通 **120 px**（AUC 0.226、置换 **p=0.003**）；框高、框高增长率（速度差代理）**无信号**。→ 给 `RULES.md` §4.2「横向间距足够小」投实测一票，且未见独立速度差要求。**效应量小**（差不到半车道，单场 AUC 0.40~0.00 抖）且 gap 锚在**画面中心**非自车——感知规格须以自车框为锚重标。
