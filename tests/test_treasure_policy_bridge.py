# -*- coding: utf-8 -*-
"""P2a-Q3b：PolicyBridge 与 module._decision_phase 契约测试。"""
...
def test_bridge_delegates_one_decision_phase():
    module = _stub_module()
    bridge = PolicyBridge(module)
    assert bridge.run(None, MagicMock()) is True
    module._decision_phase.assert_called_once_with()
...
def test_decision_phase_call_sequence_on_module():
    ...
    PolicyBridge(m).run(None, MagicMock())
...
def test_decision_phase_flushes_trace_snapshot():
    ...
    assert m._policy_snapshot is None  # 落盘后清空（与 v3 _tick_once 一致）
