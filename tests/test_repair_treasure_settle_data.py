...
另锁一个实现坑：修复后的汇总必须**从明细重建**，不能在库内旧汇总上做增量
（旧 profit_sum 是旧口径，与逐行加总本就不等，增量叠上去等于把旧口径留下——
首次实现即此错，靠落库后的复验抓出）。
"""
...
# (game_seq, auction_result, final, total, profit, income) —— 每行都是实测过的真实错型
_ROWS = [
    # ① 薄利赢单：真实利润 5,173 被旧护栏当裁位残缺丢弃记成 0
    (1, "win", 144_345, 149_518, 0, 5_173),
    # ② 动画中间值固化：利润记成 943−293,184（真值 9,798）
    (2, "win", 293_184, 302_982, -292_241, 9_798),
    # ③ 大厅场次卡串位：我方拍中却把「资产要求 300,000」当收入（真值 = 利润 90,363）
    (3, "win", 457_300, 547_663, 90_363, 300_000),
    # ④ 我方亏本拍中：收入为负是合法真值（RULES §3①），不得被改成 0
    (4, "win", 513_954, 439_085, -74_869, 300_000),
    # ⑤ 未中且中标者未亏 → 收入必须 0（旧版串位记成 300,000）
    (5, "fail", 208_000, 215_205, 7_205, 300_000),
    # ⑥ 未中且中标者亏 151,215、收入与 5/10/15% 都不符 → 推不出来，只列不改
    (7, "fail", 338_240, 187_025, -151_215, 300_000),
]
...
def _make_db(tmp_path: Path) -> Path:
    ...
    conn.executemany(
        ...
        " settle_total_price, settle_profit, settle_my_income) VALUES (?,?,?,?,?,?,?)",
        [(BUCKET, *r) for r in _ROWS],
    )
...
def test_income_repaired_only_where_rule_decides(tmp_path):
    """收入只在规则能定论处改：拍中→利润、未中且中标者未亏→0；推不出的保持原样。"""
    db = _make_db(tmp_path)
    conn = _open(db)
    plan = tool.plan_repairs(conn)
    assert len(plan["win_income_fix"]) == 2, "③④ 两场拍中收入按利润重算"
    assert len(plan["fail_income_fix"]) == 1, "⑤ 未中且无亏损 → 归零"
    ...
    assert got[7] == 300_000, "推不出来的行保持原样（不发明数据）"
    conn.close()
...
def test_summary_rebuilt_from_rows_with_new_caliber(tmp_path):
    ...
    db = _make_db(tmp_path)
    conn = _open(db)
    tool._apply(conn, tool.plan_repairs(conn), tool._summary_fix(
        conn, tool.project_summaries(conn, tool.plan_repairs(conn))))
    conn.commit()

    s = conn.execute("SELECT * FROM daily_summary").fetchone()
    # 期望值按规则算：拍中场利润 = 总价 − 成交价；未中场收入见各类处置
    win_profit = sum(r[3] - r[2] for r in _ROWS if r[1] == "win")
    assert s["profit_sum"] == win_profit == 5_173 + 9_798 + 90_363 - 74_869
    # income_sum = 逐行收入加总：四场拍中按利润、⑤ 归零、⑦ 无从推导保持原值（300,000）
    assert s["income_sum"] == win_profit + 0 + 300_000, \
        "income_sum = 逐行收入加总（未中场的分红也在内）"
    conn.close()