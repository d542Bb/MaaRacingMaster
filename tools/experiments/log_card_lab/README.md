...
- lab step 6（终态演示）：契约 §8 第 9 步落地预演——删 SECTION_ANCHORS/KW_RULES/
  前端状态推导，卡片只来自组事件，无组散行=裸行（级别色挂整行自身）；step 5 的
  散行卡徽章随锚点管线一同退役（迁移期观感，终态无散行卡）。DOM 构建全改
  textContent 落文本，无 innerHTML 注入面。实测：6 组卡徽章/行数正确、3 裸行
  （含 WARNING 琥珀）、关键词着色归零。speedrush 迁移实证：全量 1031 passed / 4 skipped。
