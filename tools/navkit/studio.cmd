@echo off
rem ============================================================================
rem  NavKit v4 Studio - C form entry (P3b): MPE IS the Studio, no shell.
...
rem  Usage:
rem    studio.cmd                          defaults (root=repo, port=26521)
rem    studio.cmd --root <dir> --port <n>  override root / port
rem    studio.cmd --mpelb <exe>            explicit mpelb path
rem    studio.cmd --stop                   kill mpelb started by this studio
...
echo [Studio] port  : %PORT%

rem only one LB at a time for this tool
taskkill /f /im mpelb.exe >nul 2>&1

start "MaaRacingAssistant Studio LB" "%MPELB%" --root "%ROOT%" --port %PORT% --log-level INFO

rem ---- P3c: 策略表薄页（独立小服务，与 MPE 并列）----
set "POLICY_PORT=26530"
set "VENV_PY=%ROOT%\.venv\Scripts\python.exe"
if exist "%VENV_PY%" if exist "%~dp0policy_server.py" (
  start "MaaRacingAssistant Studio policy" "%VENV_PY%" "%~dp0policy_server.py" --port %POLICY_PORT%
)
...
echo.
echo [Studio] Opened MPE connecting LB on port %PORT%.
echo [Studio] Opened policy table on http://127.0.0.1:%POLICY_PORT%/ (rules editor).
echo [Studio] Use "studio.cmd --stop" to stop the LocalBridge.
exit /b 0
...
:usage
echo.
echo Usage: studio.cmd [--root ^<dir^>] [--port ^<n^>] [--mpelb ^<exe^>] [--stop]
exit /b 1