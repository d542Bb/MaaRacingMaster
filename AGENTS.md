...
| **架构决策「为什么这么定」与其状态（是否仍生效）**               | [docs/adr/](docs/adr/README.md)（只收板上钉钉的架构级决策；准入与状态机见其 README）                                             |
...
**反常即差分——先立工作基线，再写第一行修复**：模块在特定阶段/场景出错时，先答「它在其余场景正常吗、正常多少次」。正常占多数时，主嫌疑在**失败场景与正常场景的差异**（上游状态、页面/时序、输入来源），不在模块内心；把差异写成一句话假设并先验它，「这段逻辑本身写错了」是最后才验的假设，不是默认假设。立基线只需一句可复算的观察，不豁免「确定要动的地方直接动手」。

**修在判据，不修在症状**：加兜底、加重试、加上限、加特例、加旋钮属于**待解释动作**——做了必须答出「为什么通用路径覆盖不到这一例」；答不出，或答案要继续列举更多场景，说明改动打错了层，停止加码。通用路径只在个别场景失效时，改**边界的输入/状态来源**（判据是否成立、门控守的是不是过期的门、真源是否滞后），而不是把通用路径再包一层。案例层见 `docs/update_log.md` 与对应域 CODE\_WIKI §9，本节不复制。

**同一方向连改三次未收敛即换轴**：同一失败在同一类假设下连续三轮未消失，该类假设即证伪——换假设的**种类**（从「模块逻辑错」换成「上下文/时序/来源不同」），或停手报告已排除范围与当前最大疑点，由用户决定是否扩大改动面。下一步若仍是「再调一个参数 / 再等长一点 / 再加一帧」，那是牛角尖信号，不是进展。
...
- **维护者待办与疑点的归宿**：开发方向与未定性疑点记入常驻台账 issue（信源路由中的 `self-todo`）——一行一条，只写「一句话 + 涉及位置」，细节不在此展开。满足任一条件即升级为独立 issue 并从台账删行：① 需跨会话追踪或与 commit 关联（`closes #N`）；② 需他人复现或讨论；③ 已进入实施、有独立验收面。已定性且另有归宿的（方向进宪法、坑点进对应域 CODE\_WIKI）直接迁走，不留副本。
